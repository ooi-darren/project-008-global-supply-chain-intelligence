...
## 17. The OECD-Sourced Backfill for Four Countries Uses a Different Pipeline, and Import-Mirror Edges Carry a Known Statistical Caveat

Items 1 and 2 describe the fix, not a residual limitation, but the fix itself has two: (1) the four backfilled countries' edges come from a different source pipeline (OECD's BTiGE, built from UN Comtrade via OECD's own institutional access) than the other 55 countries' edges (UN Comtrade's free tier directly), so a small definitional/timing mismatch between the two pipelines cannot be fully ruled out, though the real-world plausibility checks run before trusting this (`docs/SOURCES.md`) found no evidence of one. (2) Where an edge is resolved from an import mirror (e.g. France's export to the US is represented by the US's own import report from France, since France's own export report to the US does not exist in this project's data at all), this is a well-known type of statistic in international trade analysis, but mirror and direct reports of the same physical flow do not always match exactly (CIF/FOB valuation differences, reporting lags, re-exports), so these specific edges (`edge_source == "MIRROR_IMPORT_OECD"` in `trade_network_edges_export.csv`) carry marginally more uncertainty than a country's own direct export report would. Both caveats are judged acceptable given the alternative (these four countries, and every edge touching them, being absent from the network entirely, which is what this project's own experience shows actively changes downstream conclusions), but neither is hidden.
